...
| フィードバックと問題報告 | 自由記述テキスト（サーバー側で個人情報削除済み）＋仮名ユーザーハッシュ；報告種別とアプリ診断情報 | 第 6 条 (1)(a) — 同意 | 明示的な同意 |
...
| フィードバックと問題報告（オプトイン） | 個人情報を削除した自由記述テキスト＋仮名ユーザーハッシュのみ — 最長 24 か月保持し、その後自動的に消去 |
...
2. デバイスで音声を前処理（80 Hz ハイパスフィルター、-16 LUFS へのラウドネス正規化、ピークリミッティング、16 kHz リサンプリング、FLAC エンコード）
3. SafeScribe サーバーへ暗号化してアップロード（TLS 1.3）
...
| 転送中の文字起こしへの不正アクセス | 中 | TLS 1.3；SHA-256 整合性チェックサム | **低** |
