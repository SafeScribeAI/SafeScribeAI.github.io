...
<ul class="do-list">
  ...
  <li><span class="check-mark">&#x2713;</span><span class="item-body"><strong>日次バックアップ</strong><span class="item-desc">日次で上書きされるコピーは、次のバックアップサイクル後（24時間以内）にはあなたを識別できるデータを含みません。削除時点で取得される別個のポイントインタイム・リカバリのスナップショットは、災害復旧のために保持され、直ちには消去されません</span></span></li>
</ul>

削除の瞬間に、あなたの身元へのリンクはすべて破棄されます。<em>身元との関連を一切持たない</em>2つの限定的なカテゴリーのみが一時的に保持されます：財務記録（購入額・利用額）は DELETED_USER マーカーに匿名化され、瑕疵あるサービスに関する時効（トルコ消費者保護法第15条・第16条）に基づき2年間保持されます。また、ソルト付き一方向デバイスハッシュは無料クレジットの重複付与を防ぐため最長2年間保持されます（GDPR第6条1項(f)・正当な利益）。いずれも期限が切れると稼働システムから自動的に消去されます。日次で上書きされるバックアップには、24時間以内に削除済みアカウントは反映されなくなります。ただし、削除前に取得されたポイントインタイム・リカバリのスナップショットは災害復旧のために保持され、直ちには消去されません。
