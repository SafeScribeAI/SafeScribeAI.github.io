...
| Feedback e relatórios de problemas | Melhoria do produto — uma avaliação, relatório de bug, reclamação ou sugestão que você opta por enviar. O texto livre tem as DCP removidas no servidor antes do armazenamento; mantido apenas com seu hash de usuário pseudônimo (sem nome, e-mail ou IP). Retido por até 24 meses e, em seguida, expurgado automaticamente. |

<div class="callout callout-green">
  <strong>O servidor nunca armazena seu nome, endereço de e-mail, endereço IP, conteúdo de áudio ou texto de transcrição.</strong> Os únicos registros persistentes vinculados à sua conta são um hash de usuário não reversível, um identificador de dispositivo pseudônimo, um saldo de crédito, estatísticas de uso (apenas números — sem conteúdo, sem identidade) e qualquer feedback que você opte por enviar (com as DCP removidas). Se respondermos a um relatório, a resposta chega até você apenas como uma mensagem única no app, vinculada ao seu hash de usuário — nunca descobrimos quem você é.
</div>
...
<ul class="do-list">
  ...
  <li><span class="check-mark">&#x2713;</span><span class="item-body"><strong>Backup diário</strong><span class="item-desc">a cópia sobrescrita diariamente deixa de conter seus dados identificáveis após o próximo ciclo de backup (em 24 horas); um snapshot separado de recuperação point-in-time, criado no momento da exclusão, é retido para fins de recuperação de desastres e não é expurgado imediatamente</span></span></li>
</ul>

Todo vínculo com sua identidade é destruído no momento da exclusão. Duas categorias restritas persistem temporariamente <em>sem qualquer vínculo de identidade</em>: registros financeiros (valores de compra e uso) são anonimizados para o marcador DELETED_USER e mantidos por 2 anos conforme a prescrição por serviços defeituosos (Lei turca de Proteção ao Consumidor, arts. 15-16); e um hash salgado de dispositivo é mantido por até 2 anos para impedir a concessão repetida de créditos gratuitos (art. 6(1)(f) do GDPR, interesse legítimo). Ambos são expurgados automaticamente do sistema ativo ao fim do prazo. O backup sobrescrito diariamente deixa de refletir contas excluídas em 24 horas; porém, snapshots de recuperação point-in-time criados antes de uma exclusão são retidos para fins de recuperação de desastres e não são expurgados imediatamente.
...
**Removidos:** endereços de e-mail · números de telefone · endereços IP · caminhos de arquivo · tokens de autenticação
...
| Atividade de tratamento | Base GDPR | Base KVKK (Turquia) | Obrigatório? |
|------------------------|----------|----------|-------------|
